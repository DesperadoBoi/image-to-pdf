...
- `IMPORT_PDF`, `COMPRESS_PDF`, `PDF_TO_JPG`, `MERGE_PDF`;
...
`SMART_SCAN` открывает `SmartScanFragment` с CameraX preview, permission state, torch,
rule-of-thirds grid, предполагаемой рамкой документа и компактным accelerometer-level.
После снимка или одиночного системного gallery pick открывается `ScanReviewFragment` с
четырьмя ручными perspective handles.
...
→ CameraX
→ ручной perspective review
→ существующий Editor
→ PDF
```

Сохранённые `PerspectiveQuad`, `DocumentPerspectiveOverlayView`, quad validation, midpoint
geometry, perspective bitmap transform и edit spec применяются общей thumbnail/preview/PDF
pipeline. Кнопка «Авто» только восстанавливает базовую рамку; автоматическое определение
документа и auto-capture не реализованы. Полный контракт описан в
[SMART_SCAN.md](SMART_SCAN.md).
