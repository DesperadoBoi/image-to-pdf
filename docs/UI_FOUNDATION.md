...
`IMAGE_TO_PDF` запускает системный выбор нескольких изображений, `CAMERA` — существующий
системный camera flow, `MORE` открывает `AllToolsFragment`. Отмена системного выбора или
камеры ничего не меняет.
