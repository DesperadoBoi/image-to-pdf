...
`IMAGE_TO_PDF` открывает единый `ImageSourceSheet` с Gallery, Camera и Files. Gallery
использует системный Photo Picker, Files — системный Storage Access Framework, а Camera —
существующий `TakePicture`/`FileProvider` flow. Отдельный инструмент `CAMERA` сохраняет
быстрый прямой запуск камеры, `MORE` открывает `AllToolsFragment`. Отмена любого
системного выбора ничего не меняет.
...
`ImageSourceSheet` — `BottomSheetDialogFragment` на уже подключённом Material Components.
Он показывает title, subtitle и собственную иконку для Gallery, Camera и Files и возвращает
только `ImageImportSource`. `ImageImportRequest` также хранит `NEW_DOCUMENT` или
`APPEND_TO_DOCUMENT`; результат любого launcher проходит через единый session pipeline.
...
storage permissions и собственный MediaStore album browser не используются. Полный
контракт описан в [IMAGE_IMPORT_ARCHITECTURE.md](IMAGE_IMPORT_ARCHITECTURE.md).
