...
- XLSX parser использует только `java.util.zip`, `XmlPullParser` и app-cache файл. Он читает
  ...
  `SpreadsheetViewport` открывает новый лист в 100% у A1, поддерживает диагональный pan/fling,
  ручной pinch 60–300%, явный обзорный Fit width до 25%, double tap между Manual/100%/Fit
  width, закреплённые номера/заголовки и отдельное состояние каждого листа.
