...
- XLSX parser использует только `java.util.zip`, `XmlPullParser` и app-cache файл. Он читает
  ...
  `SpreadsheetViewport` поддерживает диагональный pan/fling, pinch 25–300%, double tap между
  Fit width и 100%, закреплённые номера/заголовки и отдельное состояние каждого листа.
