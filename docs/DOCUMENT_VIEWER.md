...
- XLSX parser использует только `java.util.zip`, `XmlPullParser` и app-cache файл. Он читает
  ...
  Dropdown листов и имя текущего листа переиспользуют виртуализированную по строкам grid.
  `SpreadsheetViewport` открывает новый XLSX-лист в Fit sheet overview у A1, поддерживает
  диагональный pan/fling, ручной pinch 60–300%, явные Fit width и Fit sheet до 25%, double tap
  между overview и 100%, закреплённые номера/заголовки и отдельное состояние каждого листа.
- XLSX visual fidelity includes direct/indexed/theme cell fills, basic font attributes, borders,
  alignment, wrapText, merged ranges, and worksheet column/row dimensions. XLSX sheets start in
  Fit sheet overview; 100% and Fit width remain explicit zoom modes. Row virtualization and sticky
  row/column headers remain unchanged.
