...
Эти каталоги сейчас создавать не нужно. Они зафиксированы как ориентир для дальнейшей эволюции проекта.

## Целевая UI-архитектура

- одна `Activity` как контейнер;
- отдельные `Fragment` для экранов;
- `ViewModel` для состояния пользовательской сессии;
- `LiveData` для передачи состояния в XML UI;
