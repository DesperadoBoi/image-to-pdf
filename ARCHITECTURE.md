...
- успешная операция публикует `PdfResult` и одноразовый navigation signal, привязанный к `operationId`;
- `PdfResultFragment` заменяет большой result UI редактора; в Editor остаётся только короткое действие открытия последнего результата;
- `PdfResultMetadataReader` получает display name и size через `ContentResolver` на background executor;
