...
Локальная privacy policy реализована в приложении. Статические RU/EN страницы подготовлены
для `https://desperadoboi.github.io/image-to-pdf/privacy/` и
`https://desperadoboi.github.io/image-to-pdf/privacy/en/`, но GitHub Pages нужно включить
после merge, а публичные URL затем проверить вручную. Независимо от результата текущих
проверок Data Safety повторно сверяется с точным финальным подписанным AAB.
...
| Document viewer | Выбранный или переданный системой PDF/DOCX/XLSX/text/image Uri и bounded временная копия в app cache | Нет; Share и внешний переход по `https`-ссылке только по явному действию пользователя |
...
DOCX разбирается локально. External templates/images/relationships не загружаются, macros,
VBA, ActiveX, OLE и поля-команды не выполняются. Сохранённая внешняя `https`-гиперссылка
передаётся браузеру только после явного нажатия; ImageToPDF не добавляет для viewer
`INTERNET` permission.
...
7. Не отправлять декларацию, пока GitHub Pages URL не проверены извне, developer contact не
   подтверждён и ответы не сверены с точным финальным подписанным AAB.
